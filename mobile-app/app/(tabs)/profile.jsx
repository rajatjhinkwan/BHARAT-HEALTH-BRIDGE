  ...
  return (
    <ScreenWrapper scroll={false}>
      ...
      <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
        ...
        >
          ...
          <View style={styles.cardBodyRow}>
            ...
            <PressableScale 
              ...
              style={[styles.interactiveQrContainer, { backgroundColor: scheme === 'dark' ? '#111827' : '#FFFFFF', borderColor: C.border }]}
            >
              {/* Virtual High Fidelity QR Code Grid */}
              <View style={styles.qrGrid}>
                {/* Visual rendering of a mock QR code with corner anchors */}
                <View style={styles.qrRow}>
                  <View style={[styles.qrAnchor, { backgroundColor: scheme === 'dark' ? '#FF9933' : '#1E3A8A' }]} />
                  <View style={[styles.qrDotActive, { backgroundColor: scheme === 'dark' ? '#FF9933' : '#1E3A8A' }]} />
                  <View style={styles.qrDotInactive} />
                  <View style={[styles.qrAnchor, { backgroundColor: scheme === 'dark' ? '#FF9933' : '#1E3A8A' }]} />
                </View>
                <View style={styles.qrRow}>
                  <View style={styles.qrDotInactive} />
              ...
              <Text style={[styles.qrHintText, { color: scheme === 'dark' ? '#FF9933' : '#1E3A8A' }]}>TAP TO SCAN</Text>
            </PressableScale>
          </View>
            ...
            <View style={{ flex: 1, backgroundColor: '#000' }}>
              {!cameraPermission || !cameraPermission.granted ? (
                <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', padding: 30 }}>
                  <Ionicons name="camera" size={60} color="#FF9933" style={{ marginBottom: 20 }} />
                  <Text style={{ color: '#fff', fontSize: 20, fontWeight: '800', textAlign: 'center', marginBottom: 10 }}>Camera Access Required</Text>
                  <Text style={{ color: '#aaa', textAlign: 'center', marginBottom: 30 }}>BHB App needs camera access to scan patient identity QR codes.</Text>
                  <TouchableOpacity 
                    onPress={requestCameraPermission}
                    style={{ backgroundColor: '#1E3A8A', paddingVertical: 14, paddingHorizontal: 30, borderRadius: 12 }}
                  >
                    <Text style={{ color: '#fff', fontWeight: '800' }}>Enable Camera</Text>
                  </TouchableOpacity>
                  <TouchableOpacity onPress={() => setShowScannerModal(false)} style={{ marginTop: 20 }}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <View style={{ position: 'absolute', top: 50, left: 20, right: 20, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <Text style={{ color: '#fff', fontSize: 16, fontWeight: '800' }}>Scan Patient QR Code</Text>
                    <View style={{ width: 44 }} />
                  </View>
                  ...
                  <View style={{
                    ...
                    <View style={{ position: 'absolute', top: 0, left: 0, width: 20, height: 20, borderTopWidth: 4, borderLeftWidth: 4, borderColor: '#fff' }} />
                    <View style={{ position: 'absolute', top: 0, right: 0, width: 20, height: 20, borderTopWidth: 4, borderRightWidth: 4, borderColor: '#fff' }} />
                    <View style={{ position: 'absolute', bottom: 0, left: 0, width: 20, height: 20, borderBottomWidth: 4, borderLeftWidth: 4, borderColor: '#fff' }} />
                    <View style={{ position: 'absolute', bottom: 0, right: 0, width: 20, height: 20, borderBottomWidth: 4, borderRightWidth: 4, borderColor: '#fff' }} />
                  </View>

                  <Text style={{ position: 'absolute', bottom: 100, left: 40, right: 40, color: '#fff', textAlign: 'center', fontWeight: '700', fontSize: 14, textShadowColor: 'black', textShadowOffset: { width: 1, height: 1 }, textShadowRadius: 3 }}>
                    Align the patient&apos;s identity QR code within the frame to scan.
                  </Text>
                </View>
              )}
            </View>
        ...
        {/* Scanned Decrypted Token Modal */}
        {showScannedModal && (
          <View style={[styles.modalOverlay, { backgroundColor: 'rgba(0,0,0,0.6)' }]}>
            <View style={[styles.modalContent, { backgroundColor: C.cardWhite, borderColor: C.border }]}>
              <View style={styles.modalHeader}>
                <Ionicons name="scan-circle-outline" size={32} color="#10B981" />
                <Text style={[styles.modalTitle, { color: C.textPrimary }]}>Scanned Identity Token</Text>
                <PressableScale onPress={() => setShowScannedModal(false)} style={styles.modalCloseBtn}>
                  <Ionicons name="close" size={24} color={C.textSecondary} />
                </PressableScale>
              </View>
              
              <View style={[styles.tokenContainer, { backgroundColor: C.background, borderColor: C.border, alignItems: 'center', padding: 24 }]}>
                <Ionicons name="checkmark-circle" size={48} color="#10B981" style={{ marginBottom: 12 }} />
                <Text style={{ fontSize: 11, fontWeight: '800', color: '#10B981', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 }}>
                  VERIFIED PATIENT TOKEN
                </Text>
                <Text style={{ fontSize: 22, fontWeight: '900', color: C.textPrimary, letterSpacing: 1.5 }}>
                  {formatToken(scannedToken)}
                </Text>
              </View>

              <Text style={[styles.modalHint, { color: C.textSecondary, marginBottom: 20 }]}>
                Successfully parsed unique 16-digit BHB token. Receptionist can sync clinical details directly using this secure reference.
              </Text>
              
              <PressableScale onPress={() => {
                triggerHaptic('success');
                Alert.alert("Success", "Decrypted patient token key copied to clipboard!");
                setShowScannedModal(false);
              }} style={[styles.copyBtn, { backgroundColor: C.primaryBlue }]}>
                <Ionicons name="copy-outline" size={18} color="#fff" />
                <Text style={styles.copyBtnText}>Copy Decrypted Key</Text>
              </PressableScale>
            </View>
          </View>
        )}
