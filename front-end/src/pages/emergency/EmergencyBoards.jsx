...
export default function EmergencyBoards() {
  ...
  const activeBoardMeta = boards.find((b) => b.id === activeBoard) || boards[0];
    ...
    <div className="emergency-boards-container animate-fade-in">
      <header className="boards-header">
        <div className="header-title">
          ...
          <p>Tap any patient card for full details · Live emergency tracking</p>
        </div>
        <div className="active-board-indicator" style={{ borderColor: activeBoardMeta.color }}>
          <span className="pulse-dot" style={{ backgroundColor: activeBoardMeta.color }} />
          Live Data Feed
        </div>
      </header>
      ...
      <main className="boards-grid">
        {filteredCases.length > 0 ? (
          filteredCases.map((c) => (
            <div
              key={c._id}
              className="patient-board-card patient-board-card--clickable animate-fade-in-up"
              onClick={() => openPatientDetail(c)}
            >
              <div className="card-top">
                ...
                <div className={`priority-indicator ${c.priority.toLowerCase()}`}>{c.priority}</div>
              </div>

              <div className="vitals-row">
                <div className="vital-mini"><Activity size={14} /> <span>{c.vitals?.hr || '--'}</span></div>
                <div className="vital-mini"><Thermometer size={14} /> <span>{c.vitals?.temp || '--'}</span></div>
                <div className="vital-mini"><Droplets size={14} /> <span>{c.vitals?.spO2 || '--'}</span></div>
              </div>
          ...
          <div className="empty-board">
            ...
            <h2>No Patients Currently in {activeBoardMeta.label}</h2>
      ...
      <PatientDetailOverlay
        ...
        showEmr
        onOpenEmr={(c) => navigate('/emr', { state: { emergencyCase: c } })}
        onNotifyDoctor={() => showNotification('Emergency team alerted', 'success')}
      />
    </div>
  );
}
