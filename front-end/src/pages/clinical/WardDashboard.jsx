...
export default function WardDashboard() {
  ...
  const fetchData = async () => {
    try {
      ...
      const bRes = await fetch(`${API_BASE_URL}/critical/beds?ward=${encodeURIComponent(wardName)}`);
      if (bRes.ok) {
        const bData = await bRes.json();
        // Case-insensitive filtering
        const wardBeds = bData.filter(b => b.wardName?.toUpperCase() === wardName?.toUpperCase());
        console.log(`WardDashboard DEBUG: Fetched ${wardBeds.length} beds for ${wardName}`);
        setBeds(wardBeds);
      }
    } catch (err) {
