...
import {
  ...
  Check, Filter, CheckCircle2, Info, Clock, Maximize2, X,
...
const ElectronicMedicalRecord = () => {
  ...
  const getPDFData = () => {
    return {
      patient,
      doctor: {
        name: user?.name || 'A. Sharma',
        department: user?.department || 'General Medicine',
        registrationId: user?.registrationId || 'MCI/DEL/9942/2026'
      },
      clinicalData: {
        diagnosisText: pages['Diagnosis']?.[0]?.typed || '',
        diagnosisCanvas: pages['Diagnosis']?.[0]?.content || null,
        medicineCanvas: pages['Medicine']?.[0]?.content || null,
        investigationCanvas: pages['Blood Test']?.[0]?.content || null,
        notesCanvas: pages['Notes']?.[0]?.content || null
      },
      medications: structuredMeds.filter(m => m.name),
      investigations: (pages['Blood Test']?.[0]?.typed || '').split('\n').filter(line => line.trim()),
      followUp: pages['Notes']?.[0]?.typed || 'Review after 7 days.',
  ...
  const handleUpdateQueueStatus = async (status) => {
    ...
    try {
        const endpoint = status === 'COMPLETED' ? 'complete' : 
                         status === 'TRANSFERRED' ? 'complete' : 'call-next'; 
        // We'll reuse 'complete' endpoint for transferred as well, or we can add a dedicated one.
        // For simplicity, completing the consultation on transfer is logical.
  ...
  const handleAdmitToWard = async () => {
    try {
      ...
      if (res.ok) {
        ...
        if (patient.queueId) await handleUpdateQueueStatus('TRANSFERRED');
        navigate('/doctor');
      } else {
  ...
  const handleSaveSession = async () => {
    try {
      ...
      const meds = structuredMeds.filter(m => m.name).map(m => ({
        name: m.name,
        dosage: m.dose,
        duration: m.days
      }));

      // 2. Prepare Encounter Data
      const encounter = {
        notes: pages['Notes']?.[0]?.content || 'Patient consultation completed.',
        diagnosis: pages['Diagnosis']?.[0]?.content || 'General checkup',
        prescribedBy: user?.name || 'Dr. Aryan'
      };

      // 3. Save to Backend
      const response = await fetch(`${API_BASE_URL}/clinical/patients/${patient._id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          $push: {
            prescriptions: { medications: meds, prescribedBy: user?.name || 'Dr. Aryan' },
            encounters: { ...encounter, doctorId: user?.id }
          },
          currentStatus: 'WAITING' // Ready for next action or discharge
        })
      });

      if (response.ok) {
        alert("Clinical session finalized and saved to EMR.");
        setIsZoomed(false);
  ...
  const handleServiceOrder = async (type, data) => {
    try {
      const endpoint = type === 'LAB' ? 'order-lab' : 
                       type === 'RAD' ? 'order-radiology' : 
                       type === 'SURGERY' ? 'schedule-surgery' : 'start-session';
      
      const payload = {
        patientId: patient._id,
        orderedBy: user?.name || 'Dr. Aryan',
        ...data
      };

      const response = await fetch(`${API_BASE_URL}/workflow/${endpoint}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (response.ok) {
        ...
        const refreshRes = await fetch(`${API_BASE_URL}/clinical/patients/${patient._id}`);
        if (refreshRes.ok) {
          const freshData = await refreshRes.json();
          setPatient(prev => ({ ...prev, ...freshData, currentStatus: freshData.currentStatus }));
        }
      } else {
        const err = await response.json();
        alert(err.error || `${type} order failed`);
      }
    } catch (err) {
  ...
  const addPage = () => {
    setPages(prev => {
      const newPages = { ...prev };
      newPages[activeActionTab] = [...newPages[activeActionTab], { content: null, typed: '' }];
      return newPages;
    });
    setCurrentPageIdx(pages[activeActionTab].length);
  };

  const updatePageData = (canvasData) => {
    setPages(prev => {
      const newPages = { ...prev };
      const current = newPages[activeActionTab][currentPageIdx] || { content: null, typed: '' };
      newPages[activeActionTab][currentPageIdx] = { ...current, content: canvasData };
      return newPages;
    });
  };

  const updatePageTyped = (text) => {
    setPages(prev => {
      const newPages = { ...prev };
      const current = newPages[activeActionTab][currentPageIdx] || { content: null, typed: '' };
      newPages[activeActionTab][currentPageIdx] = { ...current, typed: text };
      return newPages;
    });
  };
            ...
            <>
              <div className="input-panel non-printable">
                <div className="panel-inner p-2">
                  ...
                  {activeActionTab !== 'Medicine' && activeActionTab !== 'Referral' && (
                    <div className="p-4 space-y-4">
                       <h5 className="font-black text-[10px] uppercase text-slate-400 tracking-widest">{activeActionTab} Clinical Notes</h5>
                       <textarea 
                          placeholder={`Type ${activeActionTab} details here...`}
                          className="w-full h-64 bg-slate-50 rounded-2xl border border-slate-200 p-4 text-sm font-medium focus:outline-none focus:border-primary transition-all"
                          value={pages[activeActionTab][currentPageIdx]?.typed || ''}
                          onChange={(e) => updatePageTyped(e.target.value)}
                       />
                       <div className="flex items-center gap-3 text-slate-400 opacity-50">
                          <Edit3 size={16} />
                          <p className="text-[10px] font-bold uppercase tracking-widest italic">Handwriting also enabled on canvas</p>
                       </div>
                    </div>
                  )}
                </div>
              ...
              <div className="canvas-panel">
                ...
                <div className="a4-container-zoomable" style={{ transform: `scale(${a4Zoom})` }}>
                  <ClinicalCanvas
                    ...
                    initialData={pages[activeActionTab][currentPageIdx]?.content}
  ...
  return (
    <div className={`emr-container ${isZoomed ? 'zoom-active' : ''}`}>
      ...
      <aside className="emr-col">
        ...
        <div className="action-pad-card">
          <div className="flex justify-between items-center mb-4">
            <h3 className="pad-title">Clinical Service Orders</h3>
          ...
          <div className="pad-grid">
            <div className="pad-btn" onClick={() => handleServiceOrder('LAB', { tests: ['CBC', 'LFT', 'KFT'] })}>
              <div className="pad-btn-icon" style={{ background: 'rgba(236, 72, 153, 0.1)', color: '#ec4899' }}><FlaskConical size={18} /></div>
              <span className="pad-btn-label">Order Lab</span>
            </div>
            <div className="pad-btn" onClick={() => handleServiceOrder('RAD', { type: 'MRI', bodyPart: 'Brain' })}>
              <div className="pad-btn-icon" style={{ background: 'rgba(59, 130, 246, 0.1)', color: '#3b82f6' }}><ImageIcon size={18} /></div>
              <span className="pad-btn-label">MRI Scan</span>
            </div>
            <div className="pad-btn" onClick={() => handleServiceOrder('RAD', { type: 'X-RAY', bodyPart: 'Chest' })}>
              <div className="pad-btn-icon" style={{ background: 'rgba(99, 102, 241, 0.1)', color: '#6366f1' }}><ImageIcon size={18} /></div>
              <span className="pad-btn-label">X-Ray</span>
            </div>
            <div className="pad-btn" onClick={() => handleServiceOrder('RAD', { type: 'ULTRASOUND', bodyPart: 'Abdomen' })}>
              <div className="pad-btn-icon" style={{ background: 'rgba(20, 184, 166, 0.1)', color: '#14b8a6' }}><Activity size={18} /></div>
              <span className="pad-btn-label">Ultrasound</span>
            </div>
            <div className="pad-btn" onClick={() => handleServiceOrder('SURGERY', { procedure: 'Appendectomy', surgeon: user?.name, otNumber: 'OT-01', scheduledDate: new Date(Date.now() + 86400000) })}>
              <div className="pad-btn-icon" style={{ background: 'rgba(244, 63, 94, 0.1)', color: '#f43f5e' }}><Scissors size={18} /></div>
              <span className="pad-btn-label">Surgery</span>
            </div>
            <div className="pad-btn" onClick={() => handleServiceOrder('SESSION', { type: 'DIALYSIS', notes: 'Standard 4hr session' })}>
              <div className="pad-btn-icon" style={{ background: 'rgba(139, 92, 246, 0.1)', color: '#8b5cf6' }}><Droplets size={18} /></div>
              <span className="pad-btn-label">Dialysis</span>
            </div>
            <div className="pad-btn" onClick={() => handleServiceOrder('SESSION', { type: 'CHEMOTHERAPY', notes: 'Stage 2 Protocol' })}>
              <div className="pad-btn-icon" style={{ background: 'rgba(239, 68, 68, 0.1)', color: '#ef4444' }}><Activity size={18} /></div>
              <span className="pad-btn-label">Chemo</span>
            </div>
          </div>
        </div>
    ...
    {/* HIDDEN PRINTABLE COMPONENT - Using off-screen instead of display:none for better layout rendering */}
    <div style={{ 
      position: 'fixed', 
      top: 0, 
      left: '-100%', 
